...
class TestGetSessionData:
    ...
    def test_get_session_data_success(self):
        ...
        with patch("src.databeak.utils.session_utils.get_session_manager") as mock_manager:
            # Mock session with data
            mock_session = Mock(spec=CSVSession)
            mock_session.has_data.return_value = True
            mock_df = pd.DataFrame({"col1": [1, 2, 3], "col2": [4, 5, 6]})
            mock_session.df = mock_df

            mock_manager.return_value.get_or_create_session.return_value = mock_session
    ...
    def test_get_session_data_with_real_session(self):
        """Test get_session_data with actual CSVSession object."""
        session_id = str(uuid.uuid4())
        ...
        with patch("src.databeak.utils.session_utils.get_session_manager") as mock_manager:
            mock_manager.return_value.get_or_create_session.return_value = session
...
class TestGetSessionOnly:
    ...
    def test_get_session_only_with_real_session(self):
        """Test get_session_only with actual CSVSession object."""
        session_id = str(uuid.uuid4())

        # Create real session (without loading data)
        real_session = CSVSession(session_id)

        with patch("src.databeak.utils.session_utils.get_session_manager") as mock_manager:
            mock_manager.return_value.get_or_create_session.return_value = real_session
...
class TestValidateSessionHasData:
    ...
    def test_validate_session_has_data_with_real_session(self):
        """Test validate_session_has_data with actual CSVSession object."""
        session_id = str(uuid.uuid4())
        session = CSVSession(session_id)
...
class TestSessionUtilsRegressionPrevention:
    ...
    def test_maintains_session_manager_contract(self):
        ...
        with patch("src.databeak.utils.session_utils.get_session_manager") as mock_manager:
            ...
            mock_manager.return_value.get_or_create_session.assert_called_once_with(session_id)
    ...
    def test_error_propagation(self):
        ...
        with patch("src.databeak.utils.session_utils.get_session_manager") as mock_manager:
            ...
            mock_manager.return_value.get_or_create_session.side_effect = RuntimeError(
    ...
    def test_no_side_effects_on_session_manager(self):
        ...
        with patch("src.databeak.utils.session_utils.get_session_manager") as mock_manager:
            ...
            assert len(mock_manager.return_value.method_calls) == 1
